...
$privatekey = "6LdCaNASAAAAALR3lWoaOXHPzX6H9cd7Blv2S3ft";
if(!isset($_GET['a'])) {
...
switch(strtolower($_GET['a'])) {
	...
	case 'commitnewuser_recaptcha':
		...
		if ($resp->is_valid) {
			...
			print(JSONResponse::PrepareResponse(false, "ReCaptcha failed", $Failed));
			die();
		}
	break;
	...
	case 'insertnewstocks_v2':
		if(isset($_POST['d'])) {
			...
			$DB = new Database();
			UserManagement::InitSession();
			$DB->InsertPortfolioRows_v2($Data,$_SESSION['User']->user_id);
		}else{
			return;	
		}
	break;
