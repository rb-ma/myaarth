...
​
  ...
  <body>
    ...
    <div id="trackTotalModal" class="modal hide">
    <div class="modal-body">
    <center><div id="my_chart" style="border:0.5px; padding:10px;"></div></center>
    </div>
    <div class="modal-footer">
    	<a href="#" class="btn primary" id="closeTrackTotalModal">OK</a>
    </div>
    </div>
    ...
    <div class="container">
      <div class="content">
        ...
        <table id="preferencesTable" class="span10">
				<thead>
				<tr><th>Date</th></tr></thead><tbody><tr><td><input type="text" class="span3" id="user_date" value="<?php print(date('m/d/Y')); ?>" disabled="disabled">  <button onclick="window.location = 'settings.php';" id="modifySettings" class="btn primary2">Modify Settings</button> <button id="groupButton" class="btn">Toggle Portfolio View</button></td>
					</tr>
				</tbody>
				</table>
			<div id="tickerTrackerDiv">
				...
				<?php
				$DB = new Database();
				$data = $DB->GetUserSettings($_SESSION['User']->user_id);
				if(count($data) == 0){
				...
				if(count($Portfolio) == 0) {
					...
					foreach($Portfolio as $PortfolioItem) {
						...
						print("</select></td><td><button class='btn primary' id='stockInfo' style='font-size:12px;height:25px; width:25px; padding:0;'>?</button>  <button style='font-size:12px;height:25px; width:25px; padding:0;' class='btn trackStock'>/</button>  <button class='btn error removeStock' style='font-size:12px;height:25px; width:25px; padding:0;'>X</button></td></tr>");
					...
					print("<tr><td colspan='2' style='text-align:right;padding-top:2%;'><r5>Total </r5></td><td><div class='input-prepend'><span class='add-on btn primary unselectable disabled' style='z-index:0;'>$</span><input id='totalAmountInput' style='min-width:100px; max-width:100px;' type='text' disabled='disabled' value='".(number_format((float)$LastTotal,2,".",""))."'>  <button style='font-size:13px;height:28.5px; width:28.5px; padding:0;' class='btn trackTotal'>/</button></td><td></td><td></td></div></td></tr>");
