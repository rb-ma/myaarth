...
<html lang="en">
  ...
  <body>
    ...
    <div class="modal-body">
		<p>
			You have entered invalid stock ownership values.\r\nPlease ensure that the Dollar amounts are positive and contain only numbers.
		</p>
	</div>
    <div class="modal-footer">
		<a href="#" class="btn primary" id="cancelEmpty">Go Back</a>
    </div>
    </div>
    ...
    <div class="container">
      ...
      <div class="content">
        ...
        <div class="row">
          <div class="span11">
			<input type="search" id="searchTextBox" value="Enter Symbol" data-default-value="Enter Symbol"> <button id="finishButton" class="btn success">Save Portfolio</button> <button class="btn primary" id="refreshPage">Get Last Portfolio</button> <button id="resetPortfolioButton" class="btn error" style="z-index:99999">Empty Portfolio</button>
			...
			<div id="tickerTrackerDiv">
				...
				if(count($Portfolio) == 0) {
					...
					foreach($Portfolio as $PortfolioItem) {
						print("<tr data-tracker-index='" . $i . "'><td>" . $PortfolioItem->symbol_name . "</td><td>" . $PortfolioItem->symbol_description . "</td><td><div class='input-prepend'><span class='add-on primary toggleVal unselectable' style='z-index:0;' data-alt-value='".($PortfolioItem->ownership_type=='$'?'%':'$')."'>".$PortfolioItem->ownership_type."</span><input type='text' class='span2 stockValue' value='".(number_format((float)$PortfolioItem->ownership_value,2,".",""))."'></div></td><td><button class='btn error removeStock'>X</button></td></tr>");
					...
					if($Type == "$") {
						...
						print("<tr><td colspan='2' style='text-align:right;'>Total Value</td><td><div class='input-prepend'><span class='add-on btn primary unselectable disabled' style='z-index:0;'>$</span><input id='totalAmountInput' class='span2' type='text' value='".(number_format((float)$LastTotal,2,".",""))."'></td></div></tr>");
					}
	...
	<footer style="padding-bottom:3%"><center>&copy; myAarth LLC 2012 | <a href='mailto:contact@myaarth.com' style="color:blue;">contact@myaarth.com</a> | <button id="logout" style="background-color:white; color:blue;">Log Out</button></center></footer>
