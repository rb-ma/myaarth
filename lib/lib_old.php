...
class Database {
	...
	private function _insertPortfolioRowNew($SymbolId, $UserId, $Value, $PortfolioId,$OwnershipType) {
		if($OwnershipType == "%"){
				//have to compensate
				$new_statement = $this->MySQLi->prepare("SELECT current_portfolio_amount FROM users WHERE user_id=?");
				$new_statement->bind_param("i",$UserId);
				$new_statement->execute();
				$new_statement->bind_result($total_amount);
				while($new_statement->fetch()){
					;	
				}
				$Value = ($Value/100.0)*$total_amount;
				$new_statement->close();
		}
		if($Statement = $this->MySQLi->prepare("INSERT INTO user_stock_symbol_correlation (stock_id, user_id, last_update, ownership_value, portfolio_id, ownership_type) VALUES (?,?,NOW(),?,?,?)")) {
			$Statement->bind_param("iidis", $SymbolId, $UserId,"-1", $PortfolioId,"$");
			$Statement->execute();
			$Rows = $this->MySQLi->affected_rows;
			$Statement->close();
			return ($Rows==1 ? true : false);
	...
	public function InsertPortfolioRows($Data, $UserId) {
		...
		foreach($Data->symbols as $RowItem) {
			$new_val = $RowItem->ownership_value;
			$ownership = $RowItem->ownership_type;
			if($ownership == "%" && $total!=0.0){
				$new_val = ($RowItem->ownership_value/100.0)*$total;
				$ownership = "$";	
			}
			...
			//$this->_insertPortfolioRow((int) $RowItem->symbol_id, (int) $UserId, (double) $new_val, (int) $PortfolioId, $ownership);
