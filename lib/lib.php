...
class Database {
	...
	public function trackTotal_getData($UserId){
		...
		$this->Connect();
		$Output = array();
		if($Statement = $this->MySQLi->prepare("SELECT `portfolio_id`, `amount`, `last_change`, `timestamp` FROM graph_source where user_id = ? ORDER BY `index` ASC")){
			////ChromePhp::log("Valid Statement -- User Id: " . $UserId);
			$Statement->bind_param("i", $UserId);
			$Statement->execute();	
			$Statement->bind_result($portfolio_id, $amount, $last_change, $timestamp);
			while($Statement->fetch()){
				...
				////ChromePhp::log("$month/$day/$year: $amount, $last_change");
			}
			$Statement->close();
		} else {
			////ChromePhp::log("Invalid Statement");	
		}
		return $Output;
	}
