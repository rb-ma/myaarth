$(document).ready(function () {
	...
	$("body").on("keydown change", function(e){
	 ...
	 $("body").on("click", ".stockInfo", function () {
		 console.log("hello from stockInfo button");
		if(!modal_showing){
			modal_showing = true;
	...
	$("body").on("click", ".trackStock", function () {
		console.log("hello from trackStock");
		var symbol_index = $(this).closest("tr").attr("data-tracker-index") - 0;
		var url = "chartStock.php?i="+TickerTracker.userTrackingSymbols[symbol_index].symbol_id+"&n="+TickerTracker.userTrackingSymbols[symbol_index].symbol_name;
		window.open(url);
    });
	
	$("body").on("click", ".trackPortfolio", function () {
		var port_index = $(this).closest("tr").attr("portfolio-tracker-index") - 0;
		var name = TickerTracker.userTrackingPortfolios[port_index].name;
		var idx = -1;
		for(var i = 0; i < portfolioType.length; i++){
			if(portfolioType[i] == name){
				idx = i;
				break;
			}
		}
		idx = (idx==0)?idx:idx+1;
		var url = "chartPortfolio.php?i="+idx;
		window.open(url);
    });
	
	$("body").on("click", ".trackTotal", function () {
		window.open("chartTotal.php");
	});
	
	$("#closeTrackTotalModal").on("click", function(){
		//console.log("closing modal");
		$("#trackTotalModal").modal("hide");
		modal_showing = false;
	});
	...
	$("body").on("click", ".portfolioInfo", function () {
		if(!modal_showing){
			//console.log("able to show modal");
			modal_showing = true;
    ...
    $("#confirmModal, #showSaved, #portfolioInconsistent, #confirmTotalOwnershipEmpty, #confirmLogout, #portfolioErrorModal, #emptyModal, #refreshModal, #trackModal, #trackTotalModal, #stockInfoModal, #portfolioInfoModal").modal({
        backdrop: true,
        show: false,
        keyboard: false
    });
...
TickerTracker.userTrackingPortfolios = [];
TickerTracker.trackStock = "";
TickerTracker.trackPortfolio = "";
var num_tickers = 0;
...
TickerTracker.printPortfolioInfo = function(portfolio_id){
	//console.log("in printPortfolioInfo");
	var this_portfolio = TickerTracker.userTrackingPortfolios[portfolio_id];
	...
	output += "</r6><br/><br/><r5>Amount: $" + this_portfolio.total_value.toFixed(2) + " (" + this_portfolio.percent_of_total.toFixed(2) + "%)</r5><br/><br/>";
	
	var rtn = this_portfolio.last_change.toFixed(2);
	if(rtn > 0.0){
	...
	var rtn = this_portfolio.sofyr_return.toFixed(2);
	if(rtn > 0.0){
	...
	var rtn = this_portfolio.first_return.toFixed(2);
	if(rtn > 0.0){
...
populatePortfolios = function(){
	//console.log(TickerTracker.totalValue);
	if(TickerTracker.totalValue == 0){
		return;	
	}
	...
	for(var i = 0; i < num_portfolios; i++){
		...
		holder.percent_of_total = (holder.total_value/TickerTracker.totalValue)*100.0;
		TickerTracker.userTrackingPortfolios.push(holder);
	}
	//console.log(TickerTracker.userTrackingPortfolios);
}

TickerTracker.drawTableByPortfolio = function() {
	var output = ['<table><thead><tr><th>Portfolio Type</th><th style="text-align:right;">Amount</th><th style="text-align:right;">Return<br/>  (Yesterday)</th><th style="text-align:right;">Return<br/>(YTD)</th><th style="text-align:right;">Return<br/>(Inception)</th></tr></thead><tbody>'];
	...
	for(var i = 0; i < num_portfolios; i++){
		var curr_portfolio_type = portfolioType[i].toString();
		var holder = TickerTracker.userTrackingPortfolios[i];
		//console.log(holder);
		if(holder.total_value != 0.0){
			total_sum += holder.total_value;
			total_last_change += holder.total_value*holder.last_change;
			total_sofyr_return+= holder.total_value*holder.sofyr_return;
			total_first_return+= holder.total_value*holder.first_return;	
			...
			output.push("<td style='text-align:right'>$" + holder.total_value.toFixed(2) + "</td>");			
			
			
			if(holder.last_change > 0.0){
				output.push("<td style='text-align:right;'><font color=\"green\"><img src='Style/Images/up.png' width=\"9\" height=\"13\"/>+" + holder.last_change.toFixed(2) + "</font>%</td>");
			} else {
				output.push("<td style='text-align:right;'><font color=\"red\"><img src='Style/Images/down.png' width=\"9\" height=\"13\"/>" + holder.last_change.toFixed(2) + "</font>%</td>");
			}
			//alert("so much is working");

			
			if(holder.sofyr_return > 0.0){
				output.push("<td style='text-align:right;'><font color='green'><img src='Style/Images/up.png' width=\"9\" height=\"13\"/>+" + holder.sofyr_return.toFixed(2) + "</font>%</td>");
			} else {
				output.push("<td style='text-align:right;'><font color='red'><img src='Style/Images/down.png' width=\"9\" height=\"13\"/>" + holder.sofyr_return.toFixed(2) + "</font>%</td>");
			}
			
			if(holder.first_return > 0.0){
				output.push('<td style="text-align:right;"><font color="green"><img src="Style/Images/up.png" width=\"9\" height=\"13\"/>+' + holder.first_return.toFixed(2) + '</font>%</td><td><button class="btn primary portfolioInfo" style="font-size:12px;height:25px; width:25px; padding:0;">?</button>  <button style="font-size:12px;height:25px; width:25px; padding:0;" class="btn trackPortfolio">/</button></td></tr>');
			} else {
				output.push('<td style="text-align:right;"><font color="red"><img src="Style/Images/down.png" width=\"9\" height=\"13\"/>' + holder.first_return.toFixed(2) + '</font>%</td><td><button class="btn primary portfolioInfo" style="font-size:12px;height:25px; width:25px; padding:0;">?</button>  <button style="font-size:12px;height:25px; width:25px; padding:0;" class="btn trackPortfolio">/</button></td></tr>');
			}
		}
	}
	...
	if(total_sum > 0.0){
		...
		if(total_last_change > 0.0){
			output.push("<td style='text-align:right;'><font color='green'><img src='Style/Images/up.png' width=\"10\" height=\"16\"/><r5>+" + total_last_change.toFixed(2) + "%</r5></font></td>");
		} else {
			output.push("<td style='text-align:right;'><font color='red'><img src='Style/Images/down.png' width=\"10\" height=\"16\"/><r5>" + total_last_change.toFixed(2) + "%</r5></font></td>");
		}
		
		if(total_sofyr_return > 0.0){
			output.push("<td style='text-align:right;'><font color='green'><img src='Style/Images/up.png' width=\"10\" height=\"16\"/><r5>++" + total_sofyr_return.toFixed(2) + "%</r5></font></td>");
		} else {
			output.push("<td style='text-align:right;'><font color='red'><img src='Style/Images/down.png' width=\"10\" height=\"16\"/><r5>" + total_sofyr_return.toFixed(2) + "%</r5></font></td>");
		}
		
		if(total_first_return > 0.0){
			output.push("<td style='text-align:right;'><font color='green'><img src='Style/Images/up.png' width=\"10\" height=\"16\"/><r5>+" + total_first_return.toFixed(2) + "%</r5></font></td><td><button style='font-size:12px;height:25px; width:25px; padding:0;' class='btn trackTotal'>/</button></td></tr>");
		} else {
			output.push("<td style='text-align:right;'><font color='red'><img src='Style/Images/down.png' width=\"10\" height=\"16\"/><r5>" + total_first_return.toFixed(2) + "%</r5></font></td><td><button style='font-size:12px;height:25px; width:25px; padding:0;' class='btn trackTotal'>/</button></td></tr>");
		}
...
TickerTracker.drawTable = function () {
    ...
    $.each(TickerTracker.userTrackingSymbols, function (i, o) {
		...
		output.push("</select></td><td><button class='btn primary stockInfo' style='font-size:12px;height:25px; width:25px; padding:0;'>?</button>  <button style='font-size:12px;height:25px; width:25px; padding:0;' class='btn trackStock'>/</button>  <button class='btn error removeStock' style='font-size:12px;height:25px; width:25px; padding:0;'>X</button></tr>");
    });
	...
	if(TickerTracker.userTrackingSymbols.length != 0){
		...
		output.push("<span class='add-on btn primary unselectable disabled' style='z-index:0;'>$</span><input id='totalAmountInput' type='text' disabled='disabled' value='0.00' style='min-width:100px; max-width:100px;'>  <button style='font-size:13px;height:28.5px; width:28.5px; padding:0;' class='btn trackTotal'>/</button></td><td></td><td></td></div></td></tr>");
