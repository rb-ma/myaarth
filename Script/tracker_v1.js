$(document).ready(function () {
   Autocomplete.init($("#searchTextBox"));
    ...
    $("#confirmModal, #showSaved, #portfolioInconsistent, #confirmTotalOwnershipEmpty, #confirmLogout, #portfolioErrorModal, #emptyModal, #refreshModal, #trackModal").modal({
        backdrop: true,
        show: false,
        keyboard: true
    });
...
TickerTracker.drawTable = function () {
    ...
    $.each(TickerTracker.userTrackingSymbols, function (i, o) {
        output.push("<tr data-tracker-index='" + i + "'><td>" + o.symbol_name + "</td><td>" + o.symbol_description + "</td><td><div class='input-prepend'><span class='add-on toggleVal unselectable' data-alt-value='" + (o.ownership_type == '$' ? "%" : "$") + "' style='z-index:0;'>");
